...
class TestPredictFunctions extends FunSuite {
  ...
  test("Test date t+1 day claculations") {
    ...
    assert(PredictFunctions.getPlusOneday(List(GenericData.fields("2016-05-01", 0D, 0D, 0D, 0D, 0, 0, 0D),
      GenericData.fields("2016-04-01", 0D, 0D, 0D, 0D, 0, 0, 0D)))
  ...
  test("Test execution of steps for forecasting for Melbourne") {
      ...
      InitialProcess.downlaodObservationData(Map("MELBOURNE" -> GenericData.codes("MEL", "IDCJDW3033")), 201705, "bom")
      val x = PredictFunctions.performPrediction(Map("MELBOURNE" -> GenericData.codes("MEL", "IDCJDW3033")))(0).split(GenericData.defaultFieldSep)
      assert(x.size === 7)
      assert(x(0) === "MEL")
