...
class TestGetDataAndFileOperations extends FunSuite {
  ...
  test("Test get City to Codes Mapping") {
    ...
    val data = GetData.getMappingData.map(x => Array(x._1, x._2.IATACode, x._2.bomCode).mkString(GenericData.defaultWriteSep)).toArray
    assert(data.sorted.deep === GenericData.cities.sorted.deep)
  }

  test("File Download") {
    //Check that File for given date and bom FileName could download.
    GetData.fileDownloader(201605, GenericData.codes("CBR", "IDCJDW2801"))
  ...
  test("Parse DateRange") {
    ...
    //min and max values.
    val x = GetData.getMaxMinDate(GenericData.defaultDateRange)
