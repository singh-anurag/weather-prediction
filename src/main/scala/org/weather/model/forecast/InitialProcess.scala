...
object InitialProcess {
     private val argNamePattern = "(--)([a-zA-Z0-9_]*)".r  //Regex pattern for the key of each paramter value.
 // Function the check all mandatory parm required is supplied as arguments 
  ...
  private def validateFormatDate(year: String, month: String)(implicit argsMap: Map[String, String]): Int = {
    ...
    val dateRange = GetData.getMaxMinDate(argsMap.getOrElse("dateRange", GenericData.defaultDateRange))
   ...
   * else Do Nothing 
   */
  def downlaodObservationData(cityToCodeMapping: Map[String, GenericData.codes], date: Int, dataSource: String) = {
    dataSource match {
