...
import org.weather.model.data.{ GenericData, ReadFile, GetData }
...
object PredictFunctions {
   ...
   */
  def getPlusOneday(bomObservations: List[GenericData.fields]) = {
  ...
  def performPrediction(cityToCodeMapping: Map[String, GenericData.codes]) =
    for {
      city <- cityToCodeMapping.keys.toArray
      val bomObservations = ReadFile.getBomObservations(cityToCodeMapping(city).bomCode)
    ...
    } yield Array(cityToCodeMapping(city).IATACode, geoCordinates, tPlus1Date, outlook, "%.2f".format(predictedAverageTemp), "%.2f".format(predictedPressure), predictedRH).mkString(GenericData.defaultWriteSep)
}
